#Pangilinan, John Raphael M.
#CpE106L-B1
#Laboratory Experiment 1: Machine Problem 4
...
class ATM(EasyFrame):
    ...
    def login(self):
        ...
        elif(self.incAttemptCount >= 2):
            self.statusField.setText("Incorrect Name or PIN!")
            self.loginButton["state"] = "disabled"
            self.messageBox(title = "Login failed!", message = "Wrong PIN! Police will be notified!", width = 25, height = 5)
        elif(self.account == None):
            self.statusField.setText("Name and PIN not found!")
            self.incAttemptCount += 1
